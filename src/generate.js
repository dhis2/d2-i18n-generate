...
module.exports = function generate(args) {
    ...
    const unsupportedLangs = langs.filter((lang) => !langToLocale[lang]);

    if (unsupportedLangs.length) {
        console.error(`The following languages are not supported: ${unsupportedLangs.join(', ')}`);
        process.exit(1);
    }

    const locales = langs.map((lang) => langToLocale[lang]);
